    ...
    INotifyPropertyChanged
{
    ...
    [Export] public ItemTooltip ItemTooltip { get; set; }

    [Export] public InitiativeContainer InitiativeContainer { get; set; }
    ...
    private async void _on_start_round_pressed()
    {
        await HandleBattleround();
    }
    ...
    private void MoveTooltips(InputEventMouseMotion mouseMotion)
    {
        ...
        ItemTooltip.SetGlobalPosition(adjustedPosition);
    }
    ...
    private void MoveMousItem(InputEventMouseMotion mouseMotion)
    {
                ...
                foreach (var selection in SkillSelection)
                {
                    ...
                    if (selection.Actor.IsStunned)
                    {
                        ...
                        await WaitFor(1000);
                             ...
                             selection.Actor is BaseCreature baseCreature)
                    {
                        baseCreature.SelectedSkill = null;
                        continue;
                    }
    ...
    {
        var slotskek = InitiativeContainer.GetAllChildren<InitiativeSlot>()
            .First(s => s.AssignedUnit?.Name == selection.Actor.Name);
    ...
    private async Task ResolveDebuffs(SkillSelection selection)
    {
        ...
        foreach (var debuffs in groupedStackableDebuffs)
        {
            ...
            CombatLog.LogDebuffTick(selection.Actor, debuffs.First(), cumulatedDamage.ToString("N0"),
                remainingDuration);
        ...
        foreach (var debuff in unstackableDebuffs)
        {
            ...
            CombatLog.LogDebuffTick(selection.Actor, debuff, debuff.DamagePerTick.ToString("N0"),
    ...
    private static int ResolveEffect(IGrouping<string, Debuff> debuffs, List<Debuff> debuffsToKill,
        SkillSelection selection, out int remainingDuration)
    {
    ...
    private void ResolveSummonSkill(SkillSelection selection, BaseUnit unit, BaseSummonSkill summonSkill)
    {
    ...
    private void MachEnemiesCombatReady()
    {
        ...
        var notCombatReadyEnemies = Enemies.Where(e => (!e.IsDead &&
                                                       ...
                                                       (e.SelectedSkill is not null &&
                                                        SkillSelection.All(ss => ss.Actor.Name != e.Name)));
    ...
    private void SubscribeToInventorySlots()
    {
        ...
        foreach (var inventoryItemSlot in InventorySystem.Slots.Select(s => s.Value))
        {
            inventoryItemSlot.OnSlotLeftClicked += clickedSlot =>
            {
                ...
                else if (clickedSlot.ContainedItem is not null && mouseItemSlot.ContainedItem is not null)
                {
                    if (clickedSlot.ContainedItem.GetType() == mouseItemSlot.ContainedItem.GetType() &&
                        clickedSlot.ContainedItem is IStackable stackableItem &&
                        clickedSlot.CurrentStacksize < stackableItem.MaxStacksize)
            ...
            inventoryItemSlot.OnSlotStrgLeftClicked += clickedSlot =>
            {
                ...
                var firstStacksize = clickedSlot.CurrentStacksize % 2 == 0
                    ? clickedSlot.CurrentStacksize / 2
                    : clickedSlot.CurrentStacksize - clickedSlot.CurrentStacksize / 2;
    ...
    {
        await ToSignal(GetTree().CreateTimer((double)milliseconds / 1000), "timeout");
    }
